    ...
    public void getConnection(String schema, boolean autoCommit, RouteResultsetNode rrs,
        ...
        if (dbPool.isInitSuccess()) {
            ...
            if (rrs.getRunOnSlave() != null) {
                ...
                // 没有  /*db_type=master/slave*/ 注解，按照原来的处理方式
                LOGGER.debug("rrs.getRunOnSlave() " + rrs.getRunOnSlaveDebugInfo());
                ...
                if (rrs.canRunnINReadDB(autoCommit)) {
                    ...
                    PhysicalDatasource dbPoolSource = dbPool.getSource();
                    //记录写节点写负载值
                    dbPoolSource.setWriteCount();
                    //直接选择当前连接池中的的后端连接
                    dbPoolSource.getConnection(schema, autoCommit, handler, attachment);
                }
            }
