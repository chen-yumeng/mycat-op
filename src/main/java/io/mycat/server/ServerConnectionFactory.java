...
public class ServerConnectionFactory extends FrontendConnectionFactory {
    ...
    protected FrontendConnection getConnection(NetworkChannel channel) throws IOException {
        ...
        MycatServer.getInstance().getConfig().setSocketParams(serverConnection, true);
        serverConnection.setPrivileges(MycatPrivileges.instance());
        serverConnection.setQueryHandler(new ServerQueryHandler(serverConnection));
        serverConnection.setLoadDataInfileHandler(new ServerLoadDataInfileHandler(serverConnection));
        serverConnection.setPrepareHandler(new ServerPrepareHandler(serverConnection, sys.getMaxPreparedStmtCount()));
        serverConnection.setTxIsolation(sys.getTxIsolation());
        //创建绑定唯一Session
