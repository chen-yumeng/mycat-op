...
public class MycatConfig {
    ...
    public MycatConfig() {
        ...
        if (system.isEnableFlowControl()) {
            // 如果用户没有给定阈值，则使用我们的算法进行计算
            if (system.getFlowControlStartMaxValue() == 0 && system.getFlowControlStopMaxValue() == 0) {
                system.setFlowControlStartMaxValue((int) (Runtime.getRuntime().totalMemory() / system.getBufferPoolChunkSize() * 0.618) / dataNodes.size());
                system.setFlowControlStopMaxValue(system.getFlowControlStartMaxValue() / 5);
            }
        }
